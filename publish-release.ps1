...
if (Test-Path $changelogScript) {
    Write-Step "Generating commit changelog"
    try {
        $lastTag = (git describe --tags --abbrev=0 "$Tag^" 2>$null).Trim()
        if ([string]::IsNullOrWhiteSpace($lastTag)) {
            $lastTag = (git rev-list --max-parents=0 HEAD).Trim()
        }
        
        $changelogOutput = & $changelogScript -FromTag $lastTag -ToRef $Tag -MarkdownFormat -GroupByType
        if ($changelogOutput) {
            $changelogBlock = "`r`n`r`n$changelogOutput"
        }
    }
    catch {
        Write-Warning "Unable to generate automatic changelog: $($_.Exception.Message)"
    }
}
...
if (Test-Path (Join-Path $repoRoot $NotesFile)) {
    ...
    Set-Content -Path $tempNotesPath -Value ($notesBody + "`r`n" + $changelogBlock + "`r`n" + $compatibilityBlock)
    $cmd += @("--notes-file", $tempNotesPath)
} else {
    $cmd += @("--generate-notes", "--notes", ($notesIntro + "`r`n" + $changelogBlock + "`r`n" + $compatibilityBlock))
}
